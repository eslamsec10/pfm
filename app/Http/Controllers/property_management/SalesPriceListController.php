...
class SalesPriceListController extends Controller
{
    public function index(Request $request)
    { 
        $ids = $request->bulk_ids; 
          if ($request->bulk_action_btn === 'delete'  && is_array($ids) && count($ids)) { 
            $rentUpdated = SalePriceList::whereIn('id', $ids)->delete();
        ...
        $sales_price_list = SalePriceList::when($request['search'], function ($q) use ($request) {
        ...
        return view("admin-views.property_management.sale_price_list.sale_price_list", $data);
    }
    ...
    public function create()
    {
        ...
        return view("admin-views.property_management.sale_price_list.create_sale_price", $data);
    }

    public function edit($id)
    {
        $unit_sale            = SalePriceList::findOrFail($id);
        $property_managements = PropertyManagement::forUser()->select('id', 'name', 'code')->get();
        ...
        return view("admin-views.property_management.sale_price_list.edit_sale_price", $data);
    }
        ...
        try {
            ...
            foreach ($request->units as $unit_id) {
                ...
                SalePriceList::create([
                    ...
                    'price'         => $request->sale_price[$unit_id],
                ]);
            }
            return to_route('sales_price.index')->with('success', ui_change('added_successfully'));
        } catch (\Exception $e) {
    ...
    {
        $unit_rent = SalePriceList::findOrFail($id);
        ...
        try {
            ...
            return to_route('sales_price.index')->with('success', ui_change('updated_successfully'));
        } catch (\Exception $e) {
            return redirect()->back()->with("error", $e->getMessage());
        }
    }
    public function get_blocks_by_property_id_for_sales($id)
    {
    ...
    public function get_floors_by_block_id_for_sales($id)
    {
    ...
    public function get_units_by_floor_id_for_sales($id)
    {
    ...
    {
        $rent = SalePriceList::findOrFail($request->id);
        $rent->delete();
        return redirect()->route('sales_price.index')->with('success', ui_change('deleted_successfully'));
    }
