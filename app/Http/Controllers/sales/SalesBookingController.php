...
class SalesBookingController extends Controller
{
        ...
        try {
            ...
            $booking = SalesBooking::create([
                ...
                'booking_date'  => $request->booking_date
                    ? Carbon::createFromFormat('d/m/Y', $request->booking_date)->format('Y-m-d')
                    : null,
            ...
            return redirect()->route('sales.booking.index')
    ...
    {
        $booking                  = SalesBooking::with('booking_units', 'customer')->findOrFail($id);
        ...
        return view('admin-views.sales.bookings.add_to_agreement', $data);
    }
    ...
    public function store_to_agreement(Request $request)
    {
        ...
        try {
            $booking = SalesBooking::where('id', $request->booking_id)->first();
            ...
            $booking_units = SalesBookingUnit::where('booking_id', $request->booking_id)
                ->pluck('id');

            foreach ($booking_units as $suffix) {
