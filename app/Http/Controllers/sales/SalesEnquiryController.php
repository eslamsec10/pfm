...
class SalesEnquiryController extends Controller
{
            ...
            if ($enquiry) {
                if ($enquiry && $request->has('property_id')) {
                    ...
                    foreach ($request->property_id as $key => $property) {
                        ...
                        $sales_price         = $request->input('sales_price-' . $unit_description_id);
                        SalesEnquiryUnitSearchDetails::create([
                            ...
                            'price'                  => $request->sales_price[$key] ?? $sales_price,
                        ]);
    ...
    {
        $enquiry                  = SalesEnquiry::with('enquiry_unit_search', 'customer')->findOrFail($id);
    ...
    public function store_to_proposal(Request $request)
    {
        ...
        try {
            $proposal = SalesProposal::create([
                ...
                'proposal_date'  => ($request->proposal_date) ? Carbon::createFromFormat('d-m-Y', $request->proposal_date)->format('Y-m-d') : '',
                'customer_id'    => $request->customer_id,
                'status'         => 'proposal',
