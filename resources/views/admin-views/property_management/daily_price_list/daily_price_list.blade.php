...
@section('title', ui_change('daily_price_list'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex gap-2">
                {{ ui_change('daily_price_list') }}
                <span class="badge badge-soft-dark radius-50 fz-14 ml-1">{{ $daily_price_list->total() }}</span>
            </h2>
        </div>
                ...
                <div class="card">
                    <div class="px-3 py-4">
                        <div class="row align-items-center">
                            ...
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end">
                                <a href="{{ route('daily_price.create') }}" class="btn btn--primary">
                                    <i class="tio-add"></i>
                                    <span class="text">{{ ui_change('add_new') }}</span>
                                </a>
                            </div>
                        </div>
                    </div>
                            ...
                            <table id="datatable"
                                ...
                                <thead class="thead-light thead-50 text-capitalize">
                                    <tr>
                                        ...
                                        <th class="text-center">{{ ui_change('daily_price') }}</th>
                                        <th class="text-center">{{ ui_change('applicable_from') }}</th>
                                        <th class="text-center">{{ ui_change('Actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($daily_price_list as $k => $daily_price_list_item)
                                        <tr>
                                            <th scope="row"><input class="check_bulk_item" name="bulk_ids[]"
                                                    type="checkbox" value="{{ $daily_price_list_item->id }}" />
                                                {{ $daily_price_list->firstItem() + $k }}
                                            </th>

                                            <td class="text-center">
                                                {{ optional($daily_price_list_item->unit_management->property_unit_management)->name .
                                                    '-' .
                                                    optional($daily_price_list_item->unit_management->block_unit_management)->block->name .
                                                    '-' .
                                                    optional($daily_price_list_item->unit_management->floor_unit_management)->floor_management_main->name .
                                                    '-' .
                                                    optional($daily_price_list_item->unit_management)->unit_management_main->name ??
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center gap-2">
                                                    ...
                                                    <a class="btn btn-outline--primary btn-sm square-btn"
                                                        ...
                                                        href="{{ route('daily_price.edit', [$daily_price_list_item->id]) }}">
                                                        <i class="tio-edit"></i>
                                                    </a>
                                                    <a class="btn btn-outline-danger btn-sm delete square-btn"
                                                        title="{{ ui_change('delete') }}"
                                                        id="{{ $daily_price_list_item->id }}">
                    ...
                    <div class="table-responsive mt-4">
                        <div class="px-4 d-flex justify-content-lg-end">
                            <!-- Pagination -->
                            {{ $daily_price_list->links() }}
                        </div>
                    </div>

                    @if (count($daily_price_list) == 0)
