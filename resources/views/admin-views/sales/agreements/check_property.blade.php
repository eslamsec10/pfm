...
@php
    $lang = session()->get('locale'); //agreement_id
    $currentUrl = url()->current();
    $segments = explode('/', $currentUrl);
    $id = end($segments);
    $agreement_id = end($segments);
@endphp
        ...
        <div class="col-lg-8 col-xl-12">
            ...
            @foreach ($properties as $property_item)
                ...
                <div class="row justify-content-end gap-3 mt-3 mb-2 mx-1">
                    <a href="{{ route('agreement.image_view' , [$property_item->id , $agreement_id]) }}"   class="btn btn-secondary px-5">{{ ui_change('view_image' , 'property_transaction')  }}</a>
                    <a href="{{ route('agreement.list_view' , [$property_item->id , $agreement_id]) }}"  class="btn btn--primary px-5">{{ ui_change('list_view' , 'property_transaction')  }}</a>
                </div>
            </div>
            <hr>
            @endforeach
